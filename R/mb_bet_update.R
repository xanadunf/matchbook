...
#' @description This function provides bet update functionality. Its possible to update a simgle bet by entering in a single value for each of the runner_id, side, odds and stake parameters. It also possible to update multiple bets multiple bets at once by passing a vector for each of the runner_id, side, odds and stake parameters. In this instance, its imperative that the order in each of the vectors is preserved i.e. that the runner_id, side, odds and stake for the first update are the first elements of each parameter vector, and that the runner_id, side, odds and stake values for the second update are the second elements of each parameter vector and so on. There is a cap of 25 on the number of updates that can be placed at one time.
...
{ #mb_bet_place(session_data=session_details,runner_id=2265802,side='back',stake=2,odds=5)
  ...
  if(status_code==200)
  {
    content <- fromJSON(content(update_bet_resp, "text", "application/json"))
  } else
  {
