...
{
  ##session_data <- mb_login("xan_niallf","xan_n1allf");side=c('back','back');stake=c(2,2.3);runner_id=c(2265829,2265829); odds <- c(2.0,2.1)
  ...
  if(is.null(runner_id)){
    if(runner_id%%1>0) print(paste("The runner_id must be an integer. Please amend and try again."));return(content)
  }
  ...
  if(n_r!=length(side)|n_r!=length(stake)|n_r!=length(odds)){
    print(paste("The runner_id, stake, odds and side vectors of the same length. Please check/amend and try again."));return(content)
  }
  if(n_r>20){
