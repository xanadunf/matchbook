...
mb_get_settled <- function(session_data,sport_id=NULL,period=NULL,start_date=Sys.Date()-90,end_date=Sys.Date())
{
  ...
  get_markets_resp    <- GET(paste("https://www.matchbook.com/bpapi/rest/reports/settlements",sep=""),query=param_list,set_cookies('session-token'=session_data$session_token),add_headers('User-Agent'='rlibnf'))
  status_code        <- get_markets_resp$status_code  
  if(status_code==200)
  {
    content <- fromJSON(content(get_markets_resp, "text", "application/json"))$markets
  } else
  {
    print(paste("Warning/Error in communicating with https://www.matchbook.com/bpapi/rest/reports/settlements",event_id,"/markets",sep=""))
  }
  return(content)
}